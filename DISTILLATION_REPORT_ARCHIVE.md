...
- `distillation_project/`
    ...
    - `src/`:
        - `distillation_pipeline.py`: Simulates the Teacher (DeepSeek-V3) generating CoT reasoning and answers. **(Executed Successfully)**
...
- The `distillation_pipeline.py` script demonstrates how to clone this capability for custom domains (like Banking).

## How to Run
1.  **Generate Data**:
    ```bash
    python3 src/distillation_pipeline.py
    ```
2.  **Run Benchmark**:
    ```bash
